// Package kafka — исходящая шина auth-service: сюда складываются доменные
// события, которые сервис публикует наружу (регистрация пользователя, вход,
// выдача роли). Знания о gRPC и о базе тут нет: адаптер умеет только
// упаковать событие в конверт и положить его в топик.
package kafka
...
// Producer публикует события auth-service в Kafka.
//
...
//
// key задаёт партицию: события одного пользователя летят в одну партицию и
// потому приходят подписчику в том же порядке, в каком были опубликованы.
// Порядок между разными пользователями Kafka не гарантирует - он и не нужен.
