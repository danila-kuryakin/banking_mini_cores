// Команда migrate накатывает и откатывает схему auth-service.
//
// Конфиг читается тем же способом, что и в сервисе (configs/config.yml + .env
// + переменные окружения), поэтому запускать нужно из services/auth-service:
//
...
func main() {
	if err := run(); err != nil {
		_, _ = fmt.Fprintf(os.Stderr, "auth-service migrate: %v\n", err)
		os.Exit(1)
	}
}
