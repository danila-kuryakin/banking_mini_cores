...
import (
	...
	service "github.com/danila-kuryakin/banking_mini_cores/services/kyc-service/internal/adapters/grpc_server"
	"github.com/danila-kuryakin/banking_mini_cores/services/kyc-service/internal/adapters/kafka"
	"github.com/danila-kuryakin/banking_mini_cores/services/kyc-service/internal/config"
	kycint "github.com/danila-kuryakin/banking_mini_cores/services/kyc-service/internal/interceptors"
...
func main() {
	if err := run(); err != nil {
		_, err := fmt.Fprintf(os.Stderr, "kyc-service: %v\n", err)
		if err != nil {
			return
		}
		os.Exit(1)
	...
	return grpc_server.NewServer(
		...
		grpc_server.WithServices(
			func(r grpc.ServiceRegistrar) {
				kycv1.RegisterKycServiceServer(r, service.NewKyc(dbPool, producer, logger))
			},
		),

		// Базовую цепочку (recovery, request id, metrics, logging, timeout)
		// подключает сам grpc_server. Здесь - только интерцепторы
		// kyc-service; они встают ближе к хендлеру, уже под защитой
